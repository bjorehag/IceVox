...
class IceVoxProcessor extends AudioWorkletProcessor {
  ...
  _initEffectBuffers() {
    ...
    // ---- Echo ---- (max 1 second at the device's actual sample rate)
    // [MOBILE] Desktop hardcoded 48000; mobile devices may run at 44100 or higher,
    // so size the buffer from the AudioWorklet global `sampleRate` instead.
    this._echoBufferSize = Math.ceil(sampleRate) + 128;
    this._echoBuffers = [
