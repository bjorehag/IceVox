...
class ConnectionManager {
  ...
  async joinRoom(roomId, localStream, password = '') {
    ...
    dataConn.on('open', () => {
      ...
      // [FIX vs desktop] Send our peer-info to the host. Desktop's joinRoom never
      // does this, so the host displays raw peer IDs and its onPeerJoined callback
      // never fires for guests (guests DO get the host's info). Protocol-compatible:
      // peer-info is a normal message the host already handles.
